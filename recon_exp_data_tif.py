    ...
    for epoch in t:
        ...
        for it in range(0, len(dset), args.batch_size):
            ...
            if args.vis_freq > 0 and (total_it % args.vis_freq) == 0:
                ...
                gim.append(ax[1].imshow(y[0].detach().cpu().squeeze(), vmin=0, vmax=1, cmap='gray'))
                ax[1].axis('off')
                ax[1].title.set_text('Sim Measurement')
                gim.append(ax[2].imshow(I_est.detach().cpu().squeeze(), cmap='gray'))
                ax[2].axis('off')
                ax[2].title.set_text('I_est')
                im3=ax[3].imshow(sim_phs[0].detach().cpu().squeeze() % np.pi, cmap='rainbow')
                ax[3].axis('off')
                ax[3].title.set_text(f'Sim Phase Error at t={idx[0]}')
                gim.append(ax[4].imshow(_kernel[0].detach().cpu().squeeze(), cmap='gray'))
                ax[4].axis('off')
                ax[4].title.set_text('Sim post-SLM PSF')
                gim.append(ax[5].imshow(yy[0].squeeze().detach().cpu(), vmin=0, vmax=1, cmap='gray'))
                ax[5].axis('off')
                ax[5].title.set_text(f'Abe_est * I_est at t={idx[0]}')
                fig.colorbar(im3, cmap="rainbow", shrink=0.7)
                for im in gim:
                    fig.colorbar(im, cmap="gray", shrink=0.7)
                fig.tight_layout()
