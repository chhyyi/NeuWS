...
class TifStackDataset(Dataset):
    """
    Parse Tiff file from HW, using instead of synthetic aberration.
    It has 
    """
    def __init__(self, stacked_tif_pth, mod_tif_pth, imsize=256, num_modules=100, dtype=np.float32):
        self.obs_pth = stacked_tif_pth
        self.mod_pth = mod_tif_pth
        self.imsize = imsize

        # load observations (img)
        self.obs = tifffile.imread(self.obs_pth) #observed image with random aberration
        assert np.shape(self.obs)==(102, 256, 256), "Not Implemented..."
        self.obs = self.obs[2:].astype(dtype)/65535.0 #except two 
        self.system_corrected = self.obs[0].astype(dtype)/65535.0
        self.impact_corrected = self.obs[1].astype(dtype)/65535.0

        # load modulations
        self.mod = tifffile.imread(self.mod_pth).astype(dtype) #It is ~ 360MB.. let's just calc at init.
        assert np.shape(self.mod)==(100, 960, 960), "Not implemented..."
        self.mod = transforms.Resize((256, 256))(torch.from_numpy(self.mod))
        self.mod = torch.exp(1j * self.mod)

        def within_CTF(imsize=self.imsize, CTFsize=1.0):
            x, y = np.meshgrid(np.linspace(-1., 1., imsize), np.linspace(-1., 1., imsize))
            isin_CTF = torch.FloatTensor([[1 if i<=CTFsize else 0 for i in j] for j in np.power(np.sum((x**2, y**2), axis=0), 0.5)])
            return isin_CTF
        self.within_CTF = within_CTF(self.imsize, 1.0)
        self.mod = torch.stack([self.within_CTF*i for i in self.mod])
    ...
    def __getitem__(self, idx):
        return self.mod[idx][np.newaixs,...], self.obs[idx]
